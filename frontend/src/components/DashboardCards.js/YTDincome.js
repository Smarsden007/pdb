...
export const YTDincome = () => {
  ...
  const getTotalCost = async () => {
    try {
      const res = await axios.get("https://pdb-backend-production.up.railway.app/api/ytd-income");
...
export const MTDincome = () => {
  ...
  const getTotalCostForCurrentMonth = async () => {
    try {
      const res = await axios.get("https://pdb-backend-production.up.railway.app/api/mtd-income");
      setTotalCost(res.data.total);
    } catch (err) {
      console.error(err.response.data);
    }
...
export const SevenDayBooking = () => {
  ...
  const countBookingsWithin7Days = async () => {
    try {
      const res = await axios.get('https://pdb-backend-production.up.railway.app/api/weekly-bookings');
...
export const FourteenDayBooking = () => {
  ...
  const countBookingsWithin14Days = async () => {
    try {
      const res = await axios.get('https://pdb-backend-production.up.railway.app/api/biweekly-bookings');
      setCount(res.data.count);
    } catch (err) {
      console.error(err.response.data);
    }
...
export const UnPaid = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const result = await axios.get('https://pdb-backend-production.up.railway.app/api/unpaid');
